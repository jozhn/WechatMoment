...
function getArticleInfo() {
    ...
    mdui.prompt('公众号文章链接', function (value) {
        ...
        xhr.open('GET', 'https://www.135editor.com/downloads/getRemoteContent?uri=' + encodeURIComponent(value)); //获取标题和封面的服务器
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                if (requestDialog.getState() == 'opening' || requestDialog.getState() == 'opened') {
                    if (typeof(result) != 'undefined' && result.ret == 0) {
                        document.getElementById('configArticleTitle').value = result.data.name;
                        document.getElementById('articleTitle').innerText = result.data.name;
                        document.getElementById('articleIcon').style.backgroundImage = 'url(\"' + result.data.coverimg + '\")';
                        document.getElementById('requestResult').innerText = '获取成功！(ゝ∀･)';
                        document.getElementById('requestResultContent').innerHTML = '标题：' + result.data.name + '<br />封面：<a target="_blank" href="' + result.data.coverimg + '">点此查看</a>';
                    } else {
